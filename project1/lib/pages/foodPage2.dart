...
class _foodlistState extends State<FoodPage2> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              print(snapshot.data?.docs[2]["Menu"]); // debugging
              return ListTile(
                title: Text(snapshot.data?.docs[2]["Menu"] +
                    "\n\n" +
                    snapshot.data?.docs[2]["Description"] +
                    "\n\n" +
                    "Ingredient: " +
                    snapshot.data?.docs[2]["Ingredients"] +
                    "\n\n"),
                subtitle: Image.network(snapshot.data?.docs[2]["imageurl"]),
